/* https://github.com/Israel-D/batteri/blob/master/src/batteri.c
 * k0l0p0k190519
...
void suspend_computer(){
	g_message("Computer cannot suspend.");//заглушка
/*	UpClient* powerClient = up_client_new();
	GError* error = NULL;
	//can it suspend??
	gboolean can_suspend=up_client_get_can_suspend (powerClient);
...
void hibernate_computer(){
	g_message("Computer cannot hibernate.");//заглушка
/*	UpClient* powerClient = up_client_new();
	GError* error = NULL;
	//can it hibernate??
...
///BATTERY
static void battery_status(void* client){
// заглушка	
	return;
}
/*
...
static void power_device_alteration(void* client, void* device, gpointer data){
// заглушка	
	return;
}
///Battery stuff
...
void info_window(){
	...
	gboolean devices = FALSE;//заглушка
